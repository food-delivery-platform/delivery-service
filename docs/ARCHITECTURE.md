...
## 0. Delivery Service — Integration Addendum (read this first)

> Added 2026-07-16. This file otherwise mirrors the platform-wide `docs/ARCHITECTURE.md` verbatim so
> delivery-service engineers/agents have the full HLA in one place. This section is
> delivery-service-specific and captures where the mirrored doc, the message contracts, the data-model
> docs, and the **actual code** in sibling repos (`order-service`, `courier-frontend`) currently disagree.
> All repos are mid-development — nothing below is final. See `docs/PLAN.md` for how these decisions
> feed into implementation order.

### 0.1 Ground truth, in order of trust

When docs disagree, this is the priority order used to resolve it for delivery-service work:

1. **Working code in another repo** (e.g. `order-service/src/...`, `courier-frontend/src/...`) — what will actually be on the wire.
2. **This repo's own contracts/tests** (`docs/contracts` at platform root, `events/*.json`, scaffolded Pydantic models).
3. **Platform `docs/ARCHITECTURE.md`** — the vision doc; right on intent, sometimes stale on detail.
4. **`docs/data-model/*` snapshots** — useful for shape, least trustworthy on current enum values (see 0.3).

### 0.2 SNS/SQS: build for Standard, not FIFO

`docs/contracts/order-service/order-service-message-contracts.md` ("sprint-1" revision) specifies **SNS FIFO**
(`order-events.fifo`, `MessageGroupId`, `MessageDeduplicationId`). This contradicts platform
`ARCHITECTURE.md` §4.3/§10, which justifies **Standard** SNS/SQS on cost and throughput grounds, and
contradicts delivery-service's own `docs/contracts/delivery-service/delivery-service-message-contracts.md`,
which is written entirely against Standard SQS (`delivery-events` queue, no dedup/group IDs).

**Actual order-service code** (`order-service/src/shared/config/env.py`) has not committed to either: env vars
are `ORDER_EVENTS_TOPIC_ARN` / `ORDER_INBOUND_QUEUE_URL` with no `.fifo` naming convention, and
`events/consumer` + `events/publisher` are unimplemented stubs. **Decision for delivery-service: build the
SQS consumer against Standard semantics** (idempotent-by-design, check current state before acting, no
ordering assumed) — this matches the majority of docs and imposes no extra cost if order-service later
ships Standard. If order-service ships FIFO instead, only the queue URL/subscription changes, not the
consumer's handler logic, provided handlers stay idempotent. Re-confirm with the order-service team before
production wiring.

### 0.3 Order status: use the enum that is actually in code

Three different `order_status` enums exist across the platform right now:

| Source | Enum |
|---|---|
| Platform `ARCHITECTURE.md` §5 | `PENDING → PREPARING → READY → PICKED_UP → DELIVERED / CANCELLED / FAILED` (auto-PREPARING, no restaurant confirm step) |
| `docs/contracts/order-service-message-contracts.md` (sprint-1 draft) | `CREATED, PENDING_PAYMENT, PAID, CONFIRMED, PREPARING, READY, COURIER_ASSIGNED, PICKED_UP, DELIVERED, CANCELLED, REFUNDED, FAILED` (explicit restaurant confirm step) |
| **`order-service/src/modules/orders/model/order_status.py` (actual code, FDS-16)** | `CREATED, PENDING_PAYMENT, PAID, READY, PICKED_UP, DELIVERED, CANCELLED, FAILED` — **no `CONFIRMED`, `PREPARING`, `COURIER_ASSIGNED`, or `REFUNDED`** |

The third one is what will actually ship unless order-service changes it. Delivery Service does not own or
validate this enum — it only needs to recognize `PAID`/whatever event fires courier search, and `READY`
(triggers `order.status.ready`). **Do not hardcode assumptions about a restaurant-confirmation step inside
delivery-service** — the actual order-service code has no such state today, despite the draft contract and
platform doc disagreeing with each other on whether it exists.

### 0.4 Trigger event name for courier search: `order.preparing` is provisional

Delivery-service's own scaffold (`events/order-preparing.json`, `src/modules/events/model/order_event.py`)
assumes the inbound trigger event is `order.preparing`. But:
- `order-service-message-contracts.md` has order-service emit **`order.confirmed`** for this same moment ("payment succeeded AND restaurant accepted"), not `order.preparing`.
- Actual order-service code does not implement a publisher yet (`events/publisher/` is empty) — the real event name is undecided.

**Decision: keep `order.preparing` / `order.cancelled` / `order.status.ready` as the working assumption**
(matches this repo's own contract doc, event fixtures, and Pydantic models — least churn). Build the SQS
consumer's dispatch on the `eventType` string so adding/renaming a case later is a one-line change, and flag
this as an open cross-team question before wiring the real queue in staging.

### 0.5 `delivery.status.*` outbound: keep per-stage events, not one generic event

`order-service/src/modules/events/model/delivery_event.py` (actual code) models inbound delivery events as a
**single generic `delivery.status_changed`** event with a `stage` field. Delivery-service's own contract and
scaffold (`src/modules/events/model/delivery_event.py`) instead split this into
`delivery.status.picked_up` / `delivery.status.delivered` / `delivery.status.failed`. Keep the split version —
it carries stage-specific fields (`courierConfirmed`, `customerConfirmed`, `confirmedBy`, `failureReason`) that
the generic shape can't express, and the order-service side is unimplemented so nothing depends on the
generic shape yet. Confirm with order-service before they build their consumer.

### 0.6 Stop planning the `PATCH /orders/{orderId}/status` REST callback

Platform `ARCHITECTURE.md` §15.4, this repo's `docs/contracts/delivery-service-message-contracts.md`, and the
`README.md` all describe the Stage State Machine calling `PATCH /api/v1/orders/{orderId}/status` on Order
Service directly. **`order-service-message-contracts.md`'s revision notes explicitly removed this endpoint**:
"Delivery updates status only via SNS (consumed over SQS)". Trust the removal — it is the more recent,
specifically-reviewed decision, and a REST callback here would be a second source of truth racing the SNS
event.

**Delivery Service must notify Order Service only by publishing `delivery.status.*` events to SNS.** Do not
implement the outbound REST client for order status; delete/ignore the `PATCH /orders/{orderId}/status`
references once the publisher is built (see `docs/PLAN.md` Phase 5).

### 0.7 `active_orders` DynamoDB table: delivery-service does not own or write it

Platform `ARCHITECTURE.md` §4.7 states **Order Service** owns `active_orders` (DynamoDB) and **Delivery
Service** owns only `courier_states` + `order_events` (DynamoDB) plus `assignments` + `courier_locations`
(Supabase) — "no cross-service database writes" is a stated hard rule. Yet `ARCHITECTURE.md` §15 and this
repo's message contracts describe the Assignment Engine "writing `eligible_courier_ids` to DynamoDB
`active_orders`" — a direct violation of that same rule, and worse, `order-service`'s actual code has its own
hot table already, named **`order-status-live`** (`order-service/src/shared/config/env.py`), with a minimal
shape (`orderId, status, previousStatus, updatedAt, ttlEpoch`) that has no room for `eligible_courier_ids` at
all. `docs/data-model/dynamodb/active-orders.md` describes yet a third shape again with no
`eligible_courier_ids` field.

**Decision: Delivery Service creates and owns its own DynamoDB table for assignment/eligibility state**
(working name: `delivery_assignments`, PK `orderId`) instead of writing into any Order-Service-owned table.
It stores `eligibleCourierIds`, `assignedCourierId`, stage, and timestamps — this repo's own operational
state. Delivery Service still owns `courier_states` and `order_events` as already documented. See
`docs/PLAN.md` Phase 3 for the shape.

### 0.8 `GET /api/v1/deliveries/{orderId}` response is missing fields the courier app already needs

`courier-frontend` (already coded — see `courier-frontend/src/features/deliveries/model/Delivery.ts` and
`COURIER_APP_ARCHITECTURE.md` §9) needs `restaurantName`, `restaurantAddress`, and `customerAddress` on this
endpoint's response so the courier can navigate after accepting — fields the current
`delivery-service-message-contracts.md` example for this endpoint does not include (it only shows courier
info + status). These fields are already present on the `GET /deliveries/available` response shape, so no
new data source is needed — just carry them through. Add them to `DeliveryStateResponse` (see
`docs/PLAN.md` Phase 2).

### 0.9 Menu items live in MongoDB, not Supabase — irrelevant to delivery-service, noted to avoid confusion

`docs/data-model/mongo/menu-items.md` stores `menu_items` in MongoDB, contradicting platform
`ARCHITECTURE.md` §6 which puts `menu_items` in Supabase Postgres. Delivery Service never queries menu
items directly — item snapshots (`name`, `quantity`, `timeToPrepare`) arrive embedded in the inbound
`order.preparing` event payload. No delivery-service action needed; noted here only so an agent doesn't
add an unnecessary Mongo client while chasing this discrepancy.
...
graph TB
    subgraph CDN["Static Frontend"]
        ...
        CF2 --> S3F[S3 Bucket\nReact builds — Restaurant / Shop — Next.js static export — Courier]
